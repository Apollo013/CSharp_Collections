...
{
    public class ConcurrentStackCollection : BaseClass
    {
        public static void Run()
        {
            PrintTitle("CONCURRENT STACK");
            ...
            Parallel.For(0, 10, i =>
            {
                ...
                {
                    Console.WriteLine($"CS: Expected consecutive ranges.  range[0]={range[0]}, range[1]={range[1]}, range[2]={range[2]}");
                    Interlocked.Increment(ref errorCount);
                }
                // This will not print '1'
                Console.WriteLine($"CS: Expected consecutive ranges.  range[0]={range[0]},\trange[1]={range[1]},\trange[2]={range[2]}");
            });
